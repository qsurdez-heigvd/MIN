...
In this lab, we focus on understanding the concepts behind reinforcement learning. This is done through the use of a library, called gym, that handles the environment in which
our agent will evolve. 


## Frozen Lake Problem

Here we will study the Frozen Lake problem, which is a part of the toy series from the 
...
I observe a small difference between the formula given within the course and the one above. In deed, 
the course showcases a version without the adaptation of the old value by the 1-alpha parameter. However
in the end it's the Q-learning algorithm. The 1-alpha is just rearraging the equation for quicker computing.

- Q-learning is called off-policy, it will learn the policy without any dependance on the agent's actions. It 
will update the Q-values of the model based on the maximum possible reward for the next step. The action taken by the
agent is not relevent in this context. 

$Q(s,a) \leftarrow Q(s_t, a_t) + \alpha (r_{t+1} + \gamma \ max(Q(s_{t+1}, a') - Q(s_t, a_t))$

- SARSA is called on-policy, it will learn the policy with a total dependance on the agent's action. It will
update the Q-values of the model based on the actual action taken. 

$Q(s,a) \leftarrow Q(s_t, a_t) + \alpha (r_{t+1} + \gamma(Q(s_{t+1}, a') - Q(s_t, a_t))$

[Ref](https://www.geeksforgeeks.org/differences-between-q-learning-and-sarsa/)
...
> alpha to zero (no learning). Generate a plot of the evolution of the performance 
> as a function of interations.

I have computed the success rate of our agent every 100 epochs as to evaluate its behavior. Here's the resulting plot: 

![q-learning performance base](./PW2_notebooks/q_learning_performance_alpha0.1_gamma0.6.png)
...
![q-learning performance a1 g6](./PW2_notebooks/q_learning_performance_alpha0.1_gamma0.6.png)
![q-learning performance a1 g9](./PW2_notebooks/q_learning_performance_alpha0.1_gamma0.9.png)

![q-learning performance a05 g6](./PW2_notebooks/q_learning_performance_alpha0.05_gamma0.6.png)
![q-learning performance a05 g9](./PW2_notebooks/q_learning_performance_alpha0.05_gamma0.9.png)

![q-learning performance a01 g6](./PW2_notebooks/q_learning_performance_alpha0.01_gamma0.6.png)
![q-learning performance a01 g9](./PW2_notebooks/q_learning_performance_alpha0.01_gamma0.9.png)
...
The one difference between the graphs that is quite obvious, is how the graphs with the alpha parameter don't start around 0. No points exist between 0 and about 90. 
...
![q-learning performance a1 g6](./PW2_notebooks/q_learning_performance_alpha0.1_gamma0.6_size5.png)
![q-learning performance a1 g9](./PW2_notebooks/q_learning_performance_alpha0.1_gamma0.9_size5.png)

![q-learning performance a05 g6](./PW2_notebooks/q_learning_performance_alpha0.05_gamma0.6_size5.png)
![q-learning performance a05 g9](./PW2_notebooks/q_learning_performance_alpha0.05_gamma0.9_size5.png)

![q-learning performance a01 g6](./PW2_notebooks/q_learning_performance_alpha0.01_gamma0.6_size5.png)
![q-learning performance a01 g9](./PW2_notebooks/q_learning_performance_alpha0.01_gamma0.9_size5.png)
...
based on its past experiences. The exhaustive search will only search all possible path until it finds the right one and it is very dependent on the space in which it evolves and 
...
> r=+1 when it reaches the target. Help: https://medium.com/@ym1942/create-a-gymnasium-custom-environment-part-1-04ccc280eea9
...
![q-learning performance a1 g6](./PW2_notebooks/q_learning_performance_alpha0.1_gamma0.6_size4_reward_change.png)
![q-learning performance a1 g9](./PW2_notebooks/q_learning_performance_alpha0.1_gamma0.9_size4_reward_change.png)

![q-learning performance a05 g6](./PW2_notebooks/q_learning_performance_alpha0.05_gamma0.6_size4_reward_change.png)
![q-learning performance a05 g9](./PW2_notebooks/q_learning_performance_alpha0.05_gamma0.9_size4_reward_change.png)

![q-learning performance a01 g6](./PW2_notebooks/q_learning_performance_alpha0.01_gamma0.6_size4_reward_change.png)
![q-learning performance a01 g9](./PW2_notebooks/q_learning_performance_alpha0.01_gamma0.9_size4_reward_change.png)


The graph with the most interesting result is the one with the alpha at 0.05 and the gamma at 0.9. 
In deed, it finds an optimal path at around 6200 epochs. However if the gamma is at 0.6,
...
Here the graphs when the alpha is at 0.01, the agent doesn't converge as the alpha value
is too small to have good impact on the learning process. 

However, when the learning value is 0.1, we can clearly see that the agent finds the 
optimal solution (100% success rate). However, it finds the solution for the Q-values quicker if the 
gamme is higher, which
will value future rewards almost as much as current ones. When the gamma value is 0.6, 
...
I have plotted the success rate of the agent on the number of episodes run. Here is the graph:

![learning performance](./PW2_notebooks/taxi_images/q_learning_performance.png)
...
the end of the simulation, which explains the fact that even after that the agent has found a working
...
The results are quite interesting. We can see that when the gamma parameter is equal to 0.9, the curve 
...
much effect as soon as a solution (100% success rate) has been found and recorded in the Q-table.

The number of epochs plays an important role as well. We can understand, from the graphs, that when the
epochs are fixed to 10000, the agent doesn't find a solution for the Q-values if gamma is equal to
0.6. It takes it around 12500 epochs to find a solution for the Q-table that gives 100% success rate.

It is important to remark that, when the gamma is equal to 0.9, the curve is way steeper than in the
other graphs. Again, in this environment, future rewards are important to consider so that the agent
can find the best Q-values as quickly as possible.
...
See the code at the end of the Jupyter Notebook.
